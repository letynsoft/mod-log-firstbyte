...
* Copyright (C) 2019, Martin Letacek (http://letynsoft.com)
*
*
...
static const char *log_firstbyte_time(request_rec *r, char *a) {
  ...
  apr_time_t duration = (cf->first_out_time - cf->req_read_time);
...
static apr_status_t firstbyte_out_filter(ap_filter_t *f,
  ...
  firstbyte_config_t *cf = ap_get_module_config(f->c->conn_config,
                                                &log_firstbyte_module);
...
static int firstbyte_pre_config(apr_pool_t *p, apr_pool_t *plog,
                                apr_pool_t *ptemp) {
